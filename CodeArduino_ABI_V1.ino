...
void loop() {
  ...
  if (StatuCapteur() == 1) {
    //Serial.println("Detection du Verre ");
    Chenillard(strip.Color(  255, 0, 0), 5);
    if ((digitalRead(ButtonSelectPinte) == 1) && (digitalRead(ButtonStart) == 1) && (EtatVerre == 1)) {
      CompteurPinte(500);
      Inclinaison(TempsInclinaison);
      ChenillardRelais(strip.Color(255, 57, 0), 0, 0, 0, 1, TempsPinte);
    ...
    else if ((digitalRead(ButtonSelectPinte) == 0) && (digitalRead(ButtonStart) == 1) && (EtatVerre == 1)) {
      CompteurDemiPinte(500);
      Inclinaison(TempsInclinaison);
      ChenillardRelais(strip.Color(255, 57, 0), 0, 0, 0, 1, TempsDemiPinte);
      Descent(TempsInclinaison);
      //Chenillard(strip.Color(  255, 57, 0), TempsDemiPinte);
      statu = 0;
      int compteur = 0;
...
void Chenillard(uint32_t color, int temps) {//Paramétrage de la couleur et du temps
  for (int i = 0; i < strip.numPixels(); i++) {
    strip.setPixelColor(i, color);
    strip.show();
    delay(temps);
